"""
This script defines some functions that can be used in other scripts
...
def split_forward(model, input, size, overlap, outchannel=2, with_uncertainty=False):
    '''
    ...
    for i in range(0, h-overlap, size-overlap):
        ...
        for j in range(0, w-overlap, size-overlap):
            ...
            with torch.no_grad():
                if with_uncertainty:
                    output_patch, output2_patch = model(input_var)
                    output[:,:,ind1_s:ind1_e, ind2_s:ind2_e] = output_patch[:,:,ind1_s-i:ind1_e-i, ind2_s-j:ind2_e-j]
                    output2[:, :, ind1_s:ind1_e, ind2_s:ind2_e] = output2_patch[:,:,ind1_s-i:ind1_e-i,ind2_s-j:ind2_e-j]
                else:
